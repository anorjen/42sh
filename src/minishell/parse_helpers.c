...
//
//	j = 0;
//	new_proc->query = ft_memalloc(256);
//	while (i < position)
//	{
//		if (tokens[i][0] == '<')
//		{
...
//	j = 0;
	if (tokens[i][0] == '<')
	{
		...
		else if (strlen(tokens[i]) == 2 && tokens[i][1] == '<')
		{
			...
			strcpy(new_proc->input_path, tokens[i + 1]);
			i++;
			new_proc->type = HEREDOC_EXECUTION;
...
void		parse_helper2(process *new_proc, char **tokens, char *segment)
{
	...
	while (i < position)
	{
		...
		{
			i = parse_cycle(new_proc, tokens, i, position, j);
//			continue ;
		}
		else
		{
...
//	argc = i;
//	position = parse_cycle(new_proc, tokens, i, position, j);
//	i = argc;
	while (i <= position && new_proc->input_path == NULL)
		tokens[i++] = NULL;
