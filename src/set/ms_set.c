...
int ft_replace_set(char **args)
{
	char	**arr;
	char	*str;

	if ((arr = ft_strsplit_first_char(args[0], '=')) == NULL)
		return (0);
	set(arr[0], arr[1]);
	str = set_get(arr[0]);
	if (ft_strcmp(str, "") != 0)
...
// для димы
//	else if (argv[0] && argv[1] && argv[2] && ft_strcmp("-o", argv[1]) && (!ft_strcmp(argv[2], "vi") || !ft_strcmp(argv[2], "readline")))
//	{
//		if (!ft_strcmp(argv[2], "readline"))
//			g_input_mode = 1;
//		else
//			g_input_mode = 2;
//	}
